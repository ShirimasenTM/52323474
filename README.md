...
- If you want a different workflow you can:
	...
	- Manually add or replace images inside the `ikons/items/` (items) or `ikons/champions/` (portraits) folders and edit `index.html` / `championpicker.html` to change the `src` path if needed.
		- The repo now supports a _recommended_ organization for images under `ikons/`:
			- `ikons/champions/` — champion portrait PNGs
			- `ikons/items/` — item images used in `index.html`
			- `ikons/illustrations/` — larger site illustrations
		- A convenience PowerShell script is included to actually move files into these folders locally: `scripts/organize_ikons.ps1` (run it from the repo root; use `-DryRun` first).
...
Note: after reorganizing image files locally, open the HTML pages to verify images load correctly. The repo's HTML files have been updated to reference the `ikons/*` subfolders; run `scripts/organize_ikons.ps1` to move your files into those subfolders automatically.
